...
export async function onRequestPost(context) {
  ...
  const title = String(body.title || '').trim().slice(0, 72);
  const markdown = String(body.markdown || body.content || '');
  const handle = String(body.handle || 'user').replace(/^@/, '').toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 24);
  ...
  const record = {
    ...
    content: markdown,
    handle,
    authorName: String(body.authorName || handle).slice(0, 80),
  ...
  index.unshift({ slug, title, handle, dest: 'guide', updatedAt: now, authorName: record.authorName, authorPicture: record.authorPicture });
  ...
  pub.unshift({ slug, title, handle, dest: 'guide', updatedAt: now, authorName: record.authorName, authorPicture: record.authorPicture });
  await kv.put('guide:public', JSON.stringify(pub.slice(0, 200)));
  return json({ ok: true, slug, url: 'https://guides.trujillomingorance.com/g/' + slug }, 200);
}
